...
//        if (!::viewModel.isInitialized){
            viewModel = CoinDetailViewModel(useCases = useCases, savedStateHandle = SavedStateHandle(
                initialState = mapOf(Constants.PARAM_COIN_ID to coinRepository.coinDetail.coinId)
            ))
//        }
    }
    ...
    fun `Test loading coin detail success`() = runTest {
        ...
        assertThat(viewModel.stateFlow.value.coin).isEqualTo(coinRepository.coinDetail)
    ...
    fun `Test loading profile error`() = runTest {
        ...
        assertThat(viewModel.stateFlow.value.coin).isNull()
    ...
    fun `Test loading state updates`() = runTest {
        ...
        viewModel.stateFlow.test {
            ...
            assertEquals(viewModel.stateFlow.value.coin, emission1.coin)
            ...
            assertEquals(viewModel.stateFlow.value.coin, emission3.coin)
