...
class CoinRepositoryImpl @Inject constructor(private val api: CoinPaprikaApi,
    ...
    private fun getCoinsFromDB(): Flow<List<Coin>> {
        return  database.getCoins().map { flowCoins ->
            flowCoins.map { it.toCoin() }
        }
    }
    ...
    override suspend fun getCoins(): Flow<Resource<List<Coin>>>  = flow {
        try {
            emit(Resource.Loading())
             ...
             // Fetch coins from the API and save them to the database
